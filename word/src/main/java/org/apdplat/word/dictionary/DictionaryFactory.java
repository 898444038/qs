...
public final class DictionaryFactory {
    ...
    private static final class DictionaryHolder{
        private static Dictionary DIC = constructDictionary();
        ...
        public static void reload(){
            AutoDetector.loadAndWatch(new ResourceLoader() {
                ...
                public void load(List<String> lines) {
                    ...
                    List<String> words = RedissonService.getInstance().getRBucket("words");
                    if(words == null){
                        words = getAllWords(lines);
                        RedissonService.getInstance().setRBucket("words",words);
                    }
                    lines.clear();
                    //dic dump
                    ...
                    Dictionary dictionary = RedissonService.getInstance().getRBucket("DIC");
                    if(dictionary == null){
                        //构造词典
                        DIC.addAll(words);
                        RedissonService.getInstance().setRBucket("DIC",DIC);
                    }else{
                        DIC = dictionary;
                    }
                    //输出统计信息
