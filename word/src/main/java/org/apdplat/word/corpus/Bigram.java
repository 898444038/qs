...
import org.apdplat.word.redis.RedissonService;
...
public class Bigram {
    ...
    private static DoubleArrayGenericTrie DOUBLE_ARRAY_GENERIC_TRIE = new DoubleArrayGenericTrie(WordConfTools.getInt("bigram.double.array.trie.size", 5400000));
    private static int maxFrequency = 0;
    private static final String key = "DoubleArray";
    ...
    public static void reload(){
        ...
        AutoDetector.loadAndWatch(new ResourceLoader(){
            ...
            public void load(List<String> lines) {
                RedissonService redissonService = RedissonService.getInstance();
                LOGGER.info("初始化bigram");
                long t1 = System.currentTimeMillis();

                DoubleArrayGenericTrie doubleArrayGenericTrie = redissonService.getRBucket(key);
                if(doubleArrayGenericTrie == null){
                    ...
                    redissonService.setRBucket(key,DOUBLE_ARRAY_GENERIC_TRIE);
                }else{
                    DOUBLE_ARRAY_GENERIC_TRIE = doubleArrayGenericTrie;
                    LOGGER.info("获取缓存的双数组前缀树:"+(System.currentTimeMillis()-t1)+"ms");
                }
            }
